...
export default function Main() {
  ...
  const [flightList, setFlightList] = useState(Object.values({}));
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:5000/flight?` + 
      (condition.departure === undefined ? 'departure=ICN' : 'departure=' + condition.departure)
      + ((condition.destination === undefined || condition.destination === "") ?  '' : '&destination=' + condition.destination))
      .then(resp => resp.json())
      .then(result => {
    ...
    <div>
      ...
      <main>
        ...
        <div className="table">
          ...
          {isLoading ? <LoadingIndicator /> : <FlightList list={flightList.filter(condition)} />}
        </div>
